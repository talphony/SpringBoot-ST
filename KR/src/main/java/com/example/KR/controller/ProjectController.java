...
//                project.getCreatedBy().getId().equals(currentUser.getId()));
    ...
    public String removeMember(
        ...
        // 1. Получаем текущего пользователя и проверяем его права
        User currentUser = userService.findByUsername(principal.getName())
                .orElseThrow(() -> new UsernameNotFoundException("User not found"));
        ...
        // 2. Проверяем, является ли текущий пользователь создателем проекта
        boolean isCreator = project.getCreatedBy().equals(currentUser);

        // 3. Если не создатель, проверяем права LEAD
        if (!isCreator) {
            ...
            // Для LEAD проверяем, что удаляемый - MEMBER
            ...
            if (memberToRemove.getRole() != ProjectRole.MEMBER) {
                throw new IllegalStateException("LEAD может удалять только участников с ролью MEMBER");
            }
        }

        // 4. Выполняем удаление
        projectMemberService.removeMemberFromProject(project, userToRemove);
        redirectAttributes.addFlashAttribute("message", "Участник успешно удалён");
